    ...
    switch ($lastResponse['level']) {
        ...
        case 4:
            if ($userData === '1') {
                ...
                $amount = (($cost * 100) * 1.02);

                $url = "https://api.paystack.co/charge";
                $fields = [
                    'amount' => $amount,
                    'email' => "kwamegilbert@gmail.com", // Replace with a user-specific email if available
                    'currency' => 'GHS',
                    'mobile_money' => [
                        'phone' => $msisdn,
                ...
                curl_setopt($ch, CURLOPT_HTTPHEADER, [
                    "Authorization: Bearer sk_live_a212cee90c9ce7e37ae5179b098e5b813e0d6daf",
                ...
                if ($response_data['status'] === false || $http_code !== 200) {
                    log_error("Failed to initiate transaction: $response");
                    $message = "Failed to initiate payment. Please try again.";
                } else {
                    $response_data = json_decode($response, true);
                    if ($response_data['status'] === true && $response_data['message'] == "Charge attempted") {
                        $message = "Payment initiated successfully. Please complete the authorisation process by inputting your PIN on your mobile device";
                    } else {

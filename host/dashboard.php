...
$votes_sql = "
    ...
    FROM votes
    WHERE contestant_id IS NOT NULL AND contestant_id IN (SELECT contestant_id FROM contestants WHERE category_id IN (SELECT category_id FROM categories WHERE event_id IN (SELECT event_id FROM events WHERE host_id = ?)))
";
